...
int main()
{
    ...
    for_each(arr.begin(), arr.end(), [&sum](const int s){sum += s;});
    cout << "Sum = " << sum << endl;

    sum = std::accumulate(arr.rbegin(), arr.rend(), 0, std::plus<int>());
    cout << "Sum2 = " << sum << endl;

    arr.erase(remove_if(arr.begin(),arr.end(), [](int s){return ((s % 2) == 0);}), arr.end());

    cout << "Without even" << endl;
    for (auto it = arr.begin(); it != arr.end(); ++it)
        cout << *it << endl;
