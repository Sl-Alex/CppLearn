...
void parser(const std::vector<tBuffer> &raw, int startIndex, int endIndex, std::promise<tWordMap *> &prom)
{
...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::promise<tWordMap *> *> parserPromises;
    ...
    std::cout << "Will use future/promise for getting results from the parser" << std::endl;
    ...
    for (int i = 0; i < threadsCount; i++)
    {
        ...
        std::promise<tWordMap *> *prom = new std::promise<tWordMap *>();
        parserPromises.emplace_back(prom);
        int b,e;
        b = (rawData.size()*i)/threadsCount;
        e = (rawData.size()*(i+1))/threadsCount;
        processingPool.emplace_back(std::thread(&parser,std::ref(rawData),b,e,std::ref(*prom)));
    }

    for (int i = 0; i < threadsCount; i++)
    {
        std::future<tWordMap *> fut = parserPromises[i]->get_future();
        tWordMap * map = fut.get();
