...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Adding asyncs" << std::endl;
    for (int i = 0; i < threadsCount; i++)
    {
        ...
        //std::future<tWordMap *> fut(std::async(std::launch::async,parser,std::ref(rawData),b,e));
        //parserFutures.emplace_back(std::async(std::launch::async,parser,std::ref(rawData),b,e));
        parserFutures.emplace_back(std::async(std::launch::async,parser,std::ref(rawData),b,e));
        //processingPool.emplace_back(std::thread(std::move(*task),std::ref(rawData),b,e));
    }

    std::cout << "Getting asyncs" << std::endl;
    for (int i = 0; i < threadsCount; i++)
    {
